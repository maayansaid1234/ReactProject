...
<RemoveCircleOutlineSharpIcon ref={minus} style={{color:"palevioletred"}} onClick={()=>{amountInput.current.value--;
                  if(amountInput.current.value==0)
                  dispatch(removeFromBasket(one._id))
                else
                     dispatch(updateShoeInBasket
                      ({...one,amount:amountInput.current.value}));if(amountInput.current.value==1)
  {minus.current.style.visibility="hidden"}
  else 
  {minus.current.style.visibility="visible"}
  }}/>
